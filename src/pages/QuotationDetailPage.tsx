...
export function QuotationDetailPage() { const { id } = useParams() as { id: string };
  const quote = getQuotationById(id);
  if (!quote) return <Navigate to="/quotations" replace />;
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="lg:w-64 space-y-4">
          ...
          {quote.versions.map((v) => (
            <Card key={v.version} className={v.version === quote.versions.length ? "border-primary" : "card-hover"}>
