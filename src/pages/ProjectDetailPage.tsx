...
} from "@/components/organisms/project-tab-panels";
...
export function ProjectDetailPage() { const { id } = useParams() as { id: string };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap gap-2">
        {project.serviceTypes.map((s) => (
          <Badge key={s} variant={s === "construction" ? "secondary" : "outline"}>
      ...
      {project.constructionDetails && (
        <ConstructionDetailsPanel details={project.constructionDetails} />
      )}
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-6 mt-6">
                ...
                {projectMilestones.map((ms) => (
                  <div key={ms.id} className="flex items-center gap-4">
                    <div className={`h-3 w-3 rounded-full shrink-0 ${
                      ms.status === "completed" ? "bg-success" :
          ...
          {projectRisks.length > 0 && (
            <Card className="border-accent/30 card-hover">
              <CardHeader>
              ...
              <CardContent className="space-y-3">
                {projectRisks.map((risk) => (
                  <div key={risk.id} className="flex items-start gap-3 p-3 rounded-xl bg-accent/5">
        ...
        <TabsContent value="boq" className="mt-6">
          {projectBOQ ? (
            ...
            <p className="text-muted-foreground">No BOQ created yet.</p>
          )}
        </TabsContent>
        ...
        <TabsContent value="timeline" className="mt-6">
          <Card className="card-hover">
            <CardHeader><CardTitle className="text-lg">Project Timeline</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              {projectMilestones.length > 0 ? projectMilestones.map((ms) => (
                <div key={ms.id} className="flex items-center gap-4">
                  <div className={`h-3 w-3 rounded-full shrink-0 ${
                    ms.status === "completed" ? "bg-success" :
                    ms.status === "in_progress" ? "bg-primary" :
...
function ButtonLink({ to }: { to: string }) {
  return (
    <Link to={to} className="inline-flex mt-4 text-sm text-primary font-medium hover:underline">
