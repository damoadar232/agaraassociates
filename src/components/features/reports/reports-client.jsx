    ...
    return (<div className="space-y-6 animate-in fade-in duration-500">
      <PageHeader title="Reports" description="Analytics and business intelligence">
        <div className="flex flex-wrap gap-2">
          {["3m", "6m", "1y"].map((r) => (<Button key={r} variant={range === r ? "default" : "outline"} size="sm" className="rounded-xl" onClick={() => setRange(r)}>
              {r}
            </Button>))}
      ...
      <Tabs defaultValue="revenue">
        ...
        <TabsContent value="projects" className="mt-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((p) => (<Card key={p.id} className="card-hover">
