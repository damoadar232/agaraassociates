...
export const DashboardClient = memo(function DashboardClient({ data }: DashboardClientProps) {
  const progressPills = [
  ...
  return (
    <div className="flex flex-col gap-3 pb-20 lg:pb-0 lg:h-[calc(100vh-4.5rem)] lg:max-h-[calc(100vh-4.5rem)] animate-in fade-in duration-500">
      <DashboardWelcomeBar
        userName={data.user.name}
        studio={data.studio}
        progressPills={progressPills}
        stats={{
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-3 lg:flex-1 lg:min-h-0">
        <div className="order-1 lg:col-span-3 lg:min-h-0 lg:flex lg:flex-col">
          <DashboardHero
            ...
            onTrackPercent={data.kpis.healthScore}
          />
        </div>
        ...
        <ProjectAlertsPanel
          alerts={data.projectAlerts}
          className="order-3 lg:order-none lg:col-span-5 lg:min-h-0 lg:flex-1"
        />
        <ImmediateActionsPanel
          actions={data.immediateActions}
          className="order-4 lg:order-none lg:col-span-4 lg:min-h-0 lg:flex-1"
        />
      </div>
    </div>
  );
