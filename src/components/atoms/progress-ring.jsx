...
export function ProgressRing({ value, size = 120, strokeWidth = 8, label, className, }) {
    ...
    const color = value >= 80 ? COLORS.success :
        value >= 60 ? COLORS.accent :
            value >= 40 ? COLORS.warning :
                COLORS.danger;
    const valueTextClass = size <= 48 ? "text-xs" :
        size <= 72 ? "text-sm" :
            size <= 100 ? "text-lg" :
                "text-2xl";
    return (<div className={cn("relative inline-flex items-center justify-center", className)}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="hsl(var(--divider))" strokeWidth={strokeWidth}/>
