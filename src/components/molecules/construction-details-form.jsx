"use client";
import { CONSTRUCTION_TYPE_OPTIONS, FOUNDATION_TYPE_OPTIONS, STRUCTURAL_SYSTEM_OPTIONS, CONSTRUCTION_METHOD_OPTIONS, MACHINERY_OPTIONS, APPROVAL_OPTIONS, SAFETY_COMPLIANCE_OPTIONS, } from "@/lib/constants/onboarding";
...
function CheckboxGroup({ options, selected, onChange, }) {
    ...
    return (<div className="grid gap-2 sm:grid-cols-2">
      {options.map((option) => (<label key={option} className={cn("flex items-center gap-2 rounded-xl border px-3 py-2.5 text-sm cursor-pointer transition-colors", selected.includes(option) ? "border-primary bg-primary/5" : "hover:bg-muted/50")}>
          <input type="checkbox" checked={selected.includes(option)} onChange={() => toggle(option)} className="rounded border-input"/>
          {option}
        </label>))}
    </div>);
}
    ...
    return (<div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <FormField label="Construction Type" required>
            <Select value={value.constructionType} onChange={(e) => update("constructionType", e.target.value)}>
              {CONSTRUCTION_TYPE_OPTIONS.map((o) => (<option key={o.value} value={o.value}>{o.label}</option>))}
            </Select>
          </FormField>
          <FormField label="Foundation Type" required>
            <Select value={value.foundationType} onChange={(e) => update("foundationType", e.target.value)}>
              {FOUNDATION_TYPE_OPTIONS.map((o) => (<option key={o.value} value={o.value}>{o.label}</option>))}
            </Select>
          </FormField>
          <FormField label="Structural System" required>
            <Select value={value.structuralSystem} onChange={(e) => update("structuralSystem", e.target.value)}>
              {STRUCTURAL_SYSTEM_OPTIONS.map((o) => (<option key={o.value} value={o.value}>{o.label}</option>))}
            </Select>
          </FormField>
          <FormField label="Construction Method" required>
            <Select value={value.constructionMethod} onChange={(e) => update("constructionMethod", e.target.value)}>
              {CONSTRUCTION_METHOD_OPTIONS.map((o) => (<option key={o.value} value={o.value}>{o.label}</option>))}
            </Select>
          </FormField>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {value.milestones.map((milestone, index) => (<div key={index} className="flex flex-col gap-3 sm:flex-row sm:items-end">
              ...
              <div className="flex items-end gap-2">
                ...
                {value.milestones.length > 1 && (<Button type="button" variant="ghost" size="icon" className="shrink-0 rounded-xl" onClick={() => removeMilestone(index)}>
