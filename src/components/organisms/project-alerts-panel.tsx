...
import { ProjectAlert, ProjectAlertType } from "@/types";
...
type AlertFilter = "all" | ProjectAlertType;

const filterTypes: ProjectAlertType[] = [
  "deadline",
  "approval",
  "site_visit",
  "budget",
...
function AlertItem({ alert }: { alert: ProjectAlert }) {
  ...
  const row = (
    <div
      className={cn(
        "group relative flex gap-3 px-3 py-3 rounded-2xl border transition-colors overflow-hidden",
        style.row,
    ...
    >
      ...
      <div
        className={cn(
          "flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ml-1.5",
          style.iconBox,
        )}
      >
        ...
        <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
          ...
          <div className="flex shrink-0 items-center gap-1">
            {alert.severity === "critical" && (
              <AlertTriangle className="h-3.5 w-3.5 text-foreground" strokeWidth={1.5} />
            )}
      ...
      {alert.href && (
        <ChevronRight
          className="h-4 w-4 shrink-0 self-center text-foreground lg:opacity-0 lg:group-hover:opacity-100 transition-opacity"
          strokeWidth={1.5}
        />
      )}
    </div>
...
export function ProjectAlertsPanel({ alerts, className }: ProjectAlertsPanelProps) {
  const [filter, setFilter] = useState<AlertFilter>("all");
  const urgentCount = alerts.filter((a) => a.severity === "critical").length;
  const todayDeadlines = alerts.filter(
    (a) => a.type === "deadline" && isToday(parseISO(a.dueAt)),
  );
  ...
  return (
    <GlassCard className={cn("flex flex-col overflow-hidden lg:flex-1 lg:min-h-0", className)}>
      <div className="flex items-center justify-between gap-3 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-divider shrink-0">
        ...
        {urgentCount > 0 && (
          <Badge className="h-6 px-2.5 text-[10px] rounded-full bg-destructive/20 border border-destructive/35 text-foreground shrink-0">
            {urgentCount} urgent
          </Badge>
        )}
      </div>
      ...
      <div className="flex items-center gap-1.5 px-3 py-2.5 border-b border-divider shrink-0 overflow-x-auto scrollbar-thin">
        {filters.map((f) => {
          ...
          const typeStyle = f.id !== "all" ? alertTypeStyles[f.id as ProjectAlertType] : null;
          ...
          return (
            <button
              ...
              className={cn(
                "shrink-0 inline-flex items-center gap-1 rounded-xl px-2.5 py-1 text-[11px] font-medium transition-colors border",
                isActive && f.id === "all" && "glass-pill-active",
                isActive && typeStyle && typeStyle.filterActive,
                !isActive && "bg-surface/60 border-border/50 hover:bg-surface-hover",
              )}
            >
              {f.label}
      ...
      <div className="max-h-[340px] sm:max-h-[380px] lg:max-h-none lg:flex-1 lg:min-h-0 overflow-y-auto scrollbar-thin">
        <div className="flex flex-col gap-2 p-3">
          {filtered.length > 0 ? (
            ...
            <p className="text-xs text-center py-10 text-foreground">No alerts in this category</p>
          )}
        </div>
      </div>
    </GlassCard>
