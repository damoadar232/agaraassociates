...
import { ClientPicker, ClientPickerMode, resolveClientDisplayName } from "@/components/molecules/client-picker";
import {
  ConstructionDetailsForm,
  EMPTY_CONSTRUCTION_DETAILS,
} from "@/components/molecules/construction-details-form";
...
export function ProjectOnboardingWizard() {
  ...
  const [clientMode, setClientMode] = useState<ClientPickerMode>("existing");
  ...
  const [constructionDetails, setConstructionDetails] = useState<ConstructionDetails>(EMPTY_CONSTRUCTION_DETAILS);
  const [startDate, setStartDate] = useState("");
  const [expectedCompletion, setExpectedCompletion] = useState("");
  const [team, setTeam] = useState<string[]>([CURRENT_USER.name]);

  const hasConstruction = serviceTypes.includes("construction");

  const visibleSteps = ONBOARDING_STEPS.filter(
    (s) => s.id !== "construction" || hasConstruction
  );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {currentStep.id === "construction" && hasConstruction && (
        <ConstructionDetailsForm value={constructionDetails} onChange={setConstructionDetails} />
      )}
                ...
                {TEAM_OPTIONS.map((member) => (
                  <button
                    ...
                    className={cn(
                      ...
                      team.includes(member) ? "border-primary bg-primary/10 text-primary" : "hover:bg-muted"
                    )}
                  >
                    {member}
                  </button>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {serviceTypes.map((s) => (
                  <Badge key={s} variant={s === "construction" ? "secondary" : "default"}>
            ...
            {hasConstruction && (
              <div className="p-4 rounded-xl bg-secondary/5 border border-secondary/20 space-y-2 text-sm">
