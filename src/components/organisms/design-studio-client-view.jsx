...
export function DesignStudioClientView({ clientId }) {
    ...
    return (<>
      ...
      {drawings.length > 0 && (<Card className="border-primary/20 bg-primary/5">
      ...
      {drawings.length === 0 ? (<Card className="border-dashed">
          ...
          <TabsList>
            {types.map((t) => (<TabsTrigger key={t} value={t} className="capitalize">
                {t === "all" ? "All Files" : t.replace("_", " ")}
              </TabsTrigger>))}
          </TabsList>
          {types.map((t) => (<TabsContent key={t} value={t} className="mt-6">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {drawings
                    .filter((d) => t === "all" || d.type === t)
                    .map((d) => (<Card key={d.id} className="card-hover cursor-pointer group">
