...
export function AppHeader({ onMenuClick, glass = false }: AppHeaderProps) {
  ...
  return (
    <>
      <header className={cn(
        "sticky top-0 z-40 flex h-14 items-center gap-3 px-4 lg:px-5",
        glass
        ...
        <button
          ...
          className="flex items-center gap-2 h-10 flex-1 max-w-md rounded-xl border bg-muted/50 px-3 text-sm text-muted-foreground hover:bg-muted transition-colors"
        >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="relative rounded-xl">
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 h-4 w-4 rounded-full bg-destructive text-[10px] font-bold text-foreground flex items-center justify-center">
                    {unreadCount}
                  </span>
                )}
              </Button>
