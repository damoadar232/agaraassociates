...
interface ClientWorkspaceShellProps {
  ...
  basePath: string;
  statLabels?: { projects?: string; items?: string; itemLabel?: string };
  headerActions?: React.ReactNode;
  children: (ctx: { clientId: string; clientName: string }) => React.ReactNode;
}
...
export function ClientWorkspaceShell({
          ...
          {clients.map((client) => {
            ...
            return (
              <button
                ...
                className="text-left rounded-2xl border border-border/50 bg-surface/50 p-5 hover:bg-surface-hover hover:border-accent/30 transition-all card-hover group"
              >
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-3 min-w-0 flex-1">
          <Button
            variant="ghost"
            size="sm"
            className="rounded-xl -ml-2 h-8 px-2 text-muted-foreground"
            onClick={clearClient}
          >
      ...
      <div
        className={cn(
          "flex items-center gap-2 px-4 py-2.5 rounded-2xl border border-border/50 bg-surface/60",
        )}
      >
