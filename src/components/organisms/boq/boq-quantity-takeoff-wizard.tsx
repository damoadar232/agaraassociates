...
import { TakeoffRoomEntry } from "@/types/boq-takeoff";
...
import {
  buildTakeoffSnapshot,
  generateBoqLinesFromTakeoff,
} from "@/lib/boq/generate-boq-from-takeoff";
...
interface BoqQuantityTakeoffWizardProps {
  clientId: string;
}

export function BoqQuantityTakeoffWizard({ clientId }: BoqQuantityTakeoffWizardProps) {
  ...
  const [projectType, setProjectType] = useState<ProjectType>(
    (projects[0]?.type as ProjectType) ?? "residential",
  );
  const [quantities, setQuantities] = useState<Record<string, number>>({});
  const [rooms, setRooms] = useState<TakeoffRoomEntry[]>([DEFAULT_ROOM_ROW()]);
  ...
  const snapshot = useMemo(
    () => buildTakeoffSnapshot({ projectType, quantities, rooms, materialOverrides }),
    [projectType, quantities, rooms, materialOverrides],
  );

  const previewLines = useMemo(
    () => (catalog ? generateBoqLinesFromTakeoff(snapshot, catalog) : []),
    [snapshot, catalog],
  );

  const estimate = useMemo(() => calculateBoqEstimate(previewLines), [previewLines]);

  const setQty = (fieldId: string, value: string) => {
    setQuantities((prev) => ({
      ...prev,
  ...
  const updateRoom = (id: string, patch: Partial<TakeoffRoomEntry>) => {
    setRooms((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
        const next = { ...r, ...patch };
  ...
  const handleGenerate = (alsoQuotation: boolean) => {
  ...
  const onProjectChange = (id: string) => {
    ...
    if (project?.type) setProjectType(project.type as ProjectType);
  };

  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <Link
        to={`/boq?client=${clientId}`}
      ...
      <PageHeader
        title="Bill of Quantities — Quantity Takeoff"
        description="Fill building quantities by trade. Rates are pulled from the Master Material Library to generate your estimate."
      />
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2 max-w-2xl">
            ...
            <div className="space-y-2 sm:col-span-2">
              ...
              <Input
                ...
                placeholder="e.g. Ground Floor — Full Interior BOQ"
                            ...
                            <tr key={field.id} className="border-b hover:bg-muted/20">
                              ...
                              <td className="p-3">
                                {materials.length > 0 ? (
                                  ...
                                  <span className="text-xs text-muted-foreground">No catalog match</span>
                                )}
                              </td>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button
              ...
              onClick={() => setRooms((prev) => [...prev, DEFAULT_ROOM_ROW()])}
            >
                  ...
                  <tr key={room.id} className="border-b">
                    <td className="p-1">
                      <Input
                        ...
                        className="h-8 rounded-lg min-w-[120px]"
                      />
                    </td>
                    <td className="p-1">
                      <Input
                    ...
                    <td className="p-1">
                      <Input
                        type="number"
                        value={room.flooringQty || ""}
                        onChange={(e) => updateRoom(room.id, { flooringQty: Number(e.target.value) })}
                        className="h-8 rounded-lg w-20"
                      />
                    </td>
                    <td className="p-1">
                      <Input
                        type="number"
                        value={room.paintQty || ""}
                        onChange={(e) => updateRoom(room.id, { paintQty: Number(e.target.value) })}
                        className="h-8 rounded-lg w-20"
                      />
                    </td>
                    <td className="p-1">
                      <Input
                        type="number"
                        value={room.falseCeilingQty || ""}
                        onChange={(e) => updateRoom(room.id, { falseCeilingQty: Number(e.target.value) })}
                        className="h-8 rounded-lg w-20"
                      />
                    </td>
                    <td className="p-1">
                      <Input
                        type="number"
                        value={room.wallTilesQty || ""}
                        onChange={(e) => updateRoom(room.id, { wallTilesQty: Number(e.target.value) })}
                        className="h-8 rounded-lg w-20"
                      />
                    </td>
                    <td className="p-1">
            ...
            <Card>
              ...
              <CardContent className="p-0 overflow-x-auto max-h-[420px] overflow-y-auto">
                ...
                {previewLines.length === 0 && (
                  <p className="p-8 text-center text-muted-foreground">No quantities entered yet.</p>
                )}
              </CardContent>
            </Card>
          </div>
