...
export function QuickActionsStrip({ compact = false, className }) {
    if (compact) {
        return (<div className={cn("grid grid-cols-3 gap-1.5", className)}>
        {quickActions.map((action) => (<Link key={action.label} to={action.href} title={action.label} className="flex flex-col items-center gap-1 px-1.5 py-2 rounded-xl bg-surface/50 border border-border/40 hover:bg-surface-hover transition-colors">
    ...
    return (<div className={cn("flex items-center gap-1.5 overflow-x-auto scrollbar-thin", className)}>
      {quickActions.map((action) => (<Link key={action.label} to={action.href} title={action.label} className="group flex items-center gap-2 px-3 py-1.5 rounded-2xl bg-surface/60 border border-border/50 hover:bg-surface-hover transition-colors shrink-0">
...
export function QuickActionsCard({ className }) {
    return (<div className={cn("rounded-3xl border border-border/40 bg-surface/45 backdrop-blur-lg shadow-glass-sm px-3 py-2.5 lg:hidden", className)}>
      <p className="text-xs font-semibold text-foreground mb-2">Quick Actions</p>
      <QuickActionsStrip compact/>
    </div>);
}
export function QuickActionsPanel() {
    return (<div className="rounded-3xl border border-border/40 bg-surface/45 backdrop-blur-lg shadow-glass-sm shrink-0">
