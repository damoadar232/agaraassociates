import { ConstructionDetails } from "@/types";
import {
...
export function ConstructionDetailsPanel({ details, compact }: ConstructionDetailsPanelProps) {
  if (compact) {
    return (
  ...
  return (
    <Card className="border-secondary/20 card-hover">
      <CardHeader>
...
function TagList({ icon: Icon, title, items }: { icon: React.ComponentType<{ className?: string }>; title: string; items: string[] }) {
