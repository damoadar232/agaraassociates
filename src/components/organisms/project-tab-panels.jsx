...
export async function ProjectMaterialsTab({ projectId }) {
    ...
    return (<div className="grid gap-3 sm:grid-cols-2">
      {materials.map((m) => (<Card key={m.id} className="card-hover">
...
export function ProjectProcurementTab({ projectId }) {
    ...
    return (<div className="space-y-3">
      {orders.map((po) => (<Card key={po.id} className="card-hover">
...
export function ProjectPaymentsTab({ projectId }) {
    ...
    return (<div className="space-y-3">
      {invoices.map((inv) => (<Card key={inv.id} className="card-hover">
          <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
...
export function ProjectApprovalsTab({ projectId }) {
    ...
    return (<div className="space-y-3">
      {approvals.map((a) => (<Card key={a.id} className="card-hover">
          <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
...
export function ProjectDocumentsTab({ projectId }) {
    ...
    return (<div className="space-y-3">
      {docs.map((d) => (<Card key={d.id} className="card-hover">
          <CardContent className="p-4 flex justify-between gap-3">
            <div>
...
export function ProjectSitePhotosTab({ projectId }) {
    ...
    return (<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {photos.map((photo) => (<Card key={photo.id} className="card-hover overflow-hidden">
