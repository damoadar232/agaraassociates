...
export function ImmediateActionsPanel({ actions, className }: ImmediateActionsPanelProps) {
  const completedCount = 0;

  return (
    <GlassCard
      className={cn(
        "flex flex-col overflow-hidden border-accent/20",
      ...
      <ScrollArea className="lg:flex-1 lg:h-0 lg:min-h-0 max-h-[220px] sm:max-h-[260px] lg:max-h-none">
        <div className="space-y-0.5 p-2">
          {actions.map((action, index) => (
            <div
              key={action.id}
              className="flex items-start sm:items-center gap-2 px-2 py-2 sm:px-2.5 rounded-xl hover:bg-surface-hover transition-colors group"
            >
              {index === 0 ? (
                <CheckCircle2 className="h-3.5 w-3.5 text-accent shrink-0 mt-0.5 sm:mt-0" strokeWidth={1.5} />
              ) : (
                <Circle className="h-3.5 w-3.5 text-foreground/40 shrink-0 mt-0.5 sm:mt-0" strokeWidth={1.5} />
              )}
              ...
              <Button
                size="sm"
                variant="ghost"
                className="h-7 px-1.5 text-[10px] rounded-lg text-foreground hover:bg-surface-hover shrink-0 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity"
                asChild
              >
