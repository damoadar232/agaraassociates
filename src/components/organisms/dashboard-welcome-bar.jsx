...
export function DashboardWelcomeBar({ userName, studio, progressPills, stats, }) {
    const displayName = userName.includes(" ")
        ? userName.split(" ")[0]
        : userName;
    return (<div className="flex flex-col lg:flex-row lg:items-end gap-3 lg:gap-4 shrink-0">
      <div className="space-y-2.5 min-w-0 flex-1">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
          {progressPills.map((pill) => (<div key={pill.label} className="glass-pill flex items-center gap-2 px-2.5 sm:px-3 py-1.5 min-w-0 sm:min-w-[120px]">
              ...
              <div className="flex-1 h-1.5 rounded-full bg-surface-hover overflow-hidden min-w-[28px]">
                <div className={cn("h-full rounded-full transition-all", pill.fillClass)} style={{ width: `${Math.min(pill.value, 100)}%` }}/>
              </div>
      ...
      <div className="grid grid-cols-3 gap-2 w-full lg:flex lg:w-auto lg:ml-10 lg:mr-2 shrink-0">
        ...
        ].map((stat) => (<GlassCard key={stat.label} variant="subtle" className="px-2.5 py-2 sm:px-4 sm:py-2.5 flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-2.5 min-w-0">
