...
import { MasterMaterial, MaterialAvailability } from "@/types/material-library";
...
export function MaterialFormDialog({ open, onOpenChange, material, onSaved }: MaterialFormDialogProps) {
  ...
  const [availability, setAvailability] = useState<MaterialAvailability>(material?.availability ?? "in_stock");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: MaterialFormInput = {
      name: name.trim(),
      sku: sku.trim(),
      categoryId,
      subcategoryId,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 space-y-2">
              ...
              <Select value={availability} onChange={(e) => setAvailability(e.target.value as MaterialAvailability)} className="rounded-xl">
