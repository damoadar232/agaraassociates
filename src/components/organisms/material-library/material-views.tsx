...
import { MasterMaterial, MaterialViewMode } from "@/types/material-library";
...
function availabilityVariant(a: MasterMaterial["availability"]) {
  if (a === "in_stock") return "success" as const;
  if (a === "low_stock") return "warning" as const;
  if (a === "made_to_order") return "outline" as const;
  return "destructive" as const;
}

function MaterialThumb({ material, size = "md" }: { material: MasterMaterial; size?: "sm" | "md" | "lg" }) {
  const sizes = { sm: "h-10 w-10", md: "h-14 w-14", lg: "h-32 w-full" };
  return (
    <div
      className={cn(
        "rounded-xl bg-gradient-to-br shrink-0 flex items-center justify-center",
        material.imageGradient,
        sizes[size],
        size === "lg" && "rounded-t-2xl rounded-b-none",
      )}
    >
...
interface MaterialCardProps {
  ...
  onSelect: (m: MasterMaterial) => void;
...
export const MaterialCard = memo(function MaterialCard({
  ...
  return (
    <div
      className={cn(
        "group rounded-2xl border bg-surface/60 backdrop-blur-sm transition-all hover:shadow-glass-sm cursor-pointer overflow-hidden",
        selected ? "border-accent ring-1 ring-accent/30" : "border-border/50 hover:border-border",
        compareSelected && "ring-2 ring-info/40",
      )}
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-1">
              ...
              >
                <Star
                  className={cn("h-3.5 w-3.5", material.isFavorite ? "fill-accent text-accent" : "text-muted-foreground")}
                  strokeWidth={1.5}
                />
              </button>
            </div>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              compareSelected ? "bg-info/20 text-info" : "opacity-0 group-hover:opacity-100 hover:bg-surface-hover",
            )}
          >
...
export function MaterialViews({
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {cat.subcategories.map((sub) => (
            <button
              ...
              className="text-left p-4 rounded-2xl border border-border/50 bg-surface/50 hover:bg-surface-hover hover:border-accent/30 transition-all"
            >
  ...
  if (viewMode === "table" || viewMode === "price_list") {
    return (
      <div className="rounded-2xl border border-border/50 overflow-hidden bg-surface/40">
            ...
            <tbody>
              {materials.map((m) => (
                <tr
                  key={m.id}
                  className={cn(
                    "border-b hover:bg-surface-hover/80 cursor-pointer transition-colors",
                ...
                >
                  {viewMode === "table" && (
                    <td className="p-3">
                      <Star
                        className={cn("h-3.5 w-3.5", m.isFavorite ? "fill-accent text-accent" : "text-muted-foreground/40")}
                        onClick={(e) => {
                          e.stopPropagation();
  ...
  if (viewMode === "vendor") {
    const byVendor = materials.reduce<Record<string, MasterMaterial[]>>((acc, m) => {
  ...
  if (viewMode === "brand") {
    const byBrand = materials.reduce<Record<string, MasterMaterial[]>>((acc, m) => {
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {materials.map((m) => (
          <div
            key={m.id}
            className={cn(
              "rounded-2xl border overflow-hidden cursor-pointer transition-all hover:shadow-glass-sm",
              selectedId === m.id ? "border-accent ring-1 ring-accent/30" : "border-border/50",
            )}
            onClick={() => onSelect(m)}
          >
