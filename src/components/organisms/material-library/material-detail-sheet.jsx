...
function DetailRow({ label, value }) {
    if (value == null || value === "")
        return null;
    return (<div className="flex justify-between gap-4 py-1.5 text-sm">
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md border-l border-border/50 bg-surface/95 backdrop-blur-xl shadow-glass flex flex-col animate-in slide-in-from-right duration-300">
        <div className="flex items-center justify-between px-4 py-3 border-b border-divider shrink-0">
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onToggleFavorite(material.id)}>
              <Star className={cn("h-4 w-4", material.isFavorite ? "fill-accent text-accent" : "")} strokeWidth={1.5}/>
            </Button>
        ...
        <ScrollArea className="flex-1 min-h-0">
          <div className="p-4 space-y-4">
            <div className={cn("h-36 rounded-2xl bg-gradient-to-br flex items-end p-4", material.imageGradient)}>
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant={material.availability === "in_stock"
            ? "success"
            : material.availability === "low_stock"
                ? "warning"
                : "outline"} className="capitalize">
                {material.availability.replace(/_/g, " ")}
              </Badge>
            </div>

            {material.specification && (<p className="text-sm text-muted-foreground leading-relaxed">{material.specification}</p>)}
            ...
            {material.vendors.length > 0 && (<>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {material.vendors.map((v) => (<div key={v.id} className={cn("flex justify-between items-center p-2.5 rounded-xl border text-sm", v.isPreferred ? "border-accent/40 bg-accent/5" : "border-border/50")}>
                        <div>
            ...
            {alternates.length > 0 && (<>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {alternates.map((alt) => (<div key={alt.id} className="flex justify-between items-center p-2.5 rounded-xl border border-border/50 text-sm">
        ...
        <div className="p-4 border-t border-divider flex gap-2 shrink-0">
          ...
          <Button variant="outline" className="rounded-xl gap-1.5" size="sm" onClick={() => onArchive(material.id)}>
