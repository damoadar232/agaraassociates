...
import { Search, Plus, Upload, Download, LayoutGrid, Table2, Truck, Tag, List, ImageIcon, SlidersHorizontal, GitCompare, X, } from "lucide-react";
            ...
            : filters.quickFilter === "favorites"
                ...
                : filters.quickFilter === "recent"
                    ...
                    : filters.quickFilter === "frequent"
                        ? "Frequently Used"
                        : "All Materials";
    return (<div className="space-y-4 animate-in fade-in duration-500 pb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2">
        ...
        ].map((s) => (<div key={s.label} className="rounded-2xl border border-border/40 bg-surface/45 backdrop-blur-sm px-3 py-2.5">
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:h-[calc(100vh-16rem)] lg:min-h-[520px]">
        <div className="h-[60vh] lg:h-full lg:w-[260px] shrink-0">
          <MaterialCategorySidebar selectedCategoryId={filters.categoryId} selectedSubcategoryId={filters.subcategoryId} expandedCategories={expandedCategories} quickFilter={filters.quickFilter} categoryCounts={meta?.categoryCounts ?? {}} subcategoryCounts={meta?.subcategoryCounts ?? {}} onSelectCategory={handleSelectCategory} onToggleExpand={handleToggleExpand} onQuickFilter={(qf) => updateFilters({ quickFilter: qf, categoryId: null, subcategoryId: null })}/>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 rounded-3xl border border-border/40 bg-surface/35 backdrop-blur-sm overflow-hidden">
          <div className="p-3 sm:p-4 border-b border-divider space-y-3 shrink-0">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search by name, SKU, brand, supplier, specification..." className="pl-9 rounded-xl" value={filters.search} onChange={(e) => updateFilters({ search: e.target.value })}/>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className="rounded-xl gap-1.5">
                      ...
                      {(filters.brands.length + filters.vendors.length + filters.availability.length + (filters.priceMin != null ? 1 : 0) + (filters.priceMax != null ? 1 : 0)) > 0 && (<Badge className="h-4 px-1 text-[9px]">{filters.brands.length + filters.vendors.length + filters.availability.length + (filters.priceMin != null ? 1 : 0) + (filters.priceMax != null ? 1 : 0)}</Badge>)}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-56 max-h-80 overflow-y-auto">
                    <DropdownMenuLabel>Availability</DropdownMenuLabel>
                    {["in_stock", "low_stock", "out_of_stock", "made_to_order"].map((a) => (<DropdownMenuItem key={a} onSelect={(e) => {
                e.preventDefault();
                ...
                updateFilters({
                    ...
                    <div className="px-2 py-2 space-y-2" onClick={(e) => e.stopPropagation()}>
                      <Input type="number" placeholder="Min" className="h-8 rounded-lg text-xs" value={filters.priceMin ?? ""} onChange={(e) => updateFilters({ priceMin: e.target.value ? Number(e.target.value) : null })}/>
                      <Input type="number" placeholder="Max" className="h-8 rounded-lg text-xs" value={filters.priceMax ?? ""} onChange={(e) => updateFilters({ priceMax: e.target.value ? Number(e.target.value) : null })}/>
                    </div>
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel>Brand</DropdownMenuLabel>
                    {brands.slice(0, 12).map((b) => (<DropdownMenuItem key={b} onSelect={(e) => {
                e.preventDefault();
                ...
                updateFilters({
                    ...
                    {vendors.slice(0, 10).map((v) => (<DropdownMenuItem key={v} onSelect={(e) => {
                e.preventDefault();
                ...
                {compareIds.length > 0 && (<Button variant="secondary" size="sm" className="rounded-xl gap-1.5" onClick={() => setShowCompare(true)}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1 overflow-x-auto scrollbar-thin pb-0.5">
                {VIEW_MODES.map((mode) => (<button key={mode.id} type="button" onClick={() => setViewMode(mode.id)} className={cn("shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[11px] font-medium border transition-colors", viewMode === mode.id
                ? "glass-pill-active"
          ...
          <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin p-3 sm:p-4">
            {loading ? (<div className="space-y-2">
                {Array.from({ length: 8 }).map((_, i) => (<div key={i} className="h-12 rounded-xl bg-muted/40 animate-pulse"/>))}
              </div>) : (<>
                <MaterialViews materials={filteredMaterials} viewMode={viewMode} selectedId={selectedMaterial?.id ?? null} compareIds={compareIds} categoryId={filters.categoryId} subcategoryId={filters.subcategoryId} subcategoryCounts={meta?.subcategoryCounts ?? {}} onSelect={setSelectedMaterial} onToggleFavorite={handleToggleFavorite} onToggleCompare={handleToggleCompare} onSelectSubcategory={(catId, subId) => {
                handleSelectCategory(catId, subId);
                setViewMode("table");
            }}/>
                {totalPages > 1 && (viewMode === "table" || viewMode === "price_list" || viewMode === "gallery") && (<div className="mt-4 flex items-center justify-between border-t border-divider pt-4">
      ...
      {selectedMaterial && (<>
          ...
          <MaterialDetailSheet material={selectedMaterial} onClose={() => setSelectedMaterial(null)} onToggleFavorite={handleToggleFavorite} onArchive={async (id) => {
      ...
      {showCompare && compareMaterials.length > 0 && (<div className="fixed inset-x-0 bottom-0 z-50 p-4 lg:pl-[280px]">
          <div className="rounded-2xl border border-border/50 bg-surface/95 backdrop-blur-xl shadow-glass p-4 max-w-5xl mx-auto">
            ...
            <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
              {compareMaterials.map((m) => (<div key={m.id} className="p-3 rounded-xl border border-border/50 text-sm space-y-1">
