...
const seedDocuments: ProjectDocument[] = [
...
let documents: ProjectDocument[] = [...seedDocuments];

function syncDrawingDocuments() {
  const drawingDocs: ProjectDocument[] = getAllDrawings().map((d) => {
  ...
  const invoiceDocs: ProjectDocument[] = invoices.map((inv) => ({
    id: `doc-inv-${inv.id}`,
...
export function getDocumentsByProjectId(projectId: string) {
  return documents.filter((d) => d.projectId === projectId);
}

export function addDocument(doc: Omit<ProjectDocument, "id">) {
  const newDoc: ProjectDocument = { ...doc, id: `doc-${Date.now()}` };
