...
export function getDrawingsByProjectId(projectId: string) {
  return drawings.filter((d) => d.projectId === projectId);
}

export function getDrawingsByProjectIds(projectIds: string[]) {
  return drawings.filter((d) => projectIds.includes(d.projectId));
}

export function addDrawing(input: CreateDrawingInput): Drawing {
